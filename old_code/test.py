...
for i in range(0, len(sigmas)):
    ...
    reconstruction_errors_sigmas[i, simple_NMF] = 0.5 * np.sum((V - np.dot(W, H))**2)/np.sum(V**2)
    print("Finished simple NMF")
    W, H = NMF.non_negative_sparse_matrix_factorization(V, k, sparseness_coefficients)
    reconstruction_errors_sigmas[i, sparse_NMF] =  0.5 * np.sum((V - np.dot(W, H))**2)/np.sum(V**2)
...
for i in range(0, len(number_of_atoms)):
    ...
    reconstruction_errors_atoms[i, simple_NMF] = 0.5 * np.sum((V - np.dot(W, H))**2)/np.sum(V**2)
    print("Finished simple NMF")
    W, H = NMF.non_negative_sparse_matrix_factorization(V, number_of_atoms[i], sparseness_coefficients)
    reconstruction_errors_atoms[i, sparse_NMF] =  0.5 * np.sum((V - np.dot(W, H))**2)/np.sum(V**2)
    print("Finished sparse NMF")    
    #W, H = NMF.nmf_sparsness_constraint_hoyer(V, k, sparseness_atoms, sparseness_coefficients)
    #reconstruction_errors[i, NMF_sparseness_constraint] =  0.5 * np.sum((V - np.dot(W, H))**2)
...
print("Obtained all data")
fig = pp.figure()
pp.plot(number_of_atoms, reconstruction_errors_atoms[:, simple_NMF], label = "NMF Lee and Seung 2001")
pp.plot(number_of_atoms, reconstruction_errors_atoms[:, sparse_NMF], label = "NMF Hoyer 2002")
